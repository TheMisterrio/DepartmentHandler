...
def departments():
    """
    Sends get request to server (api/departments); receives list of departments from server and displays it
...
def department(dep_id):
    ...
    POST method: takes new department`s id, sends post request to server that updates department`s id in database

    :param str dep_id: department`s id which is used to find department
    :return: GET: HTML page with department`s id that for changing | POST: redirect to departments page after updating
    """
...
def add_department():
    ...
    POST method: sends data about department into server (api/department) that adds new department in database

    :return: HTML page with form to add new department | POST: redirect to departments page after adding
    """
    if request.method == 'POST':
...
def employees():
    """
    GET method: sends get request to server (api/employees); receives list of employees from server and displays it
    POST method: sends post request to server (api/employees) with dates from form; receives list of employees that has
    been filtered with dates - employees who were born on a certain date or in the period between dates
    ...
    if request.method == 'POST':
        ...
        logger.warning(date_from)
        logger.warning(date_by)
...
def employee(employee_id):
    """
    GET method: sends get request to server (/api/employee<employee_id>) and get request to server (/api/departments-ids)
    that returns list of departments` ids; receives information about employee and displays it in form for changing
    employee`s data; departments` ids put into HTML tag selection for choice
    POST method: gets information about employee from form and sends it into server that updates info in database
    ...
    if request.method == 'POST':
        ...
        requests.put(url, data={'name': name, 'department_id': int(department_id), 'date_of_birthday': date_of_birthday,
                                'salary': int(salary)})
...
def add_employee():
    """
    GET method: sends get request to server (/api/departments-ids) that returns list of departments` ids; displays form
    to write information about new employee; departments` ids put into HTML tag selection for choice
    POST method: gets information about new employee from form and sends it into server that adds new employee in database
    ...
    if request.method == 'POST':
        ...
        requests.post('http://127.0.0.1:5000/api/employee',
                      data={'name': name, 'department_id': department_id, 'date_of_birthday': date_of_birthday,
                            'salary': salary})
        return redirect('/employees')
    ids = requests.get('http://127.0.0.1:5000/api/departments-ids').json()['ids']
